...
#####----------------------- Part 1: Revenue and Transaction -----------------------#####

# Questions: 
# How well is Google Merchandise Store doing from 2016-08-01 to 2017-07-31?
...
# Insights and questions:
    # Transaction and Revnue was at its peak late 2016 (November and December)
    # Was this the trend? Why is google merchandise store performing bad as time goes? 
...
google_traffic_insights = google %>%
  ...
  group_by(newVisits) %>%
  summarise(total_pageviews = sum(pageviews), total_timeOnSite = sum(timeOnSite), total_hits = sum(hits))
